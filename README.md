...
Python ve Streamlit ile yerel voleybol video analizi. MVP doğrudan voleybolla açılır; branş seçimi yoktur. RTMPose-L WholeBody ile hareket adayları ve deneysel ölçümler üretilir. Fiziksel doğruluk henüz bağımsız çekimlerle doğrulanmadı.
...
CLI: `.venv/bin/python main.py --help`. Varsayılan giriş `data/videos/sample.mp4`, çıktılar `data/output/`. Arayüz analizleri `data/analyses.sqlite3` ve `data/analyses/` altında kalıcıdır. Voleybol çalışma alanındaki **Analiz geçmişi → Kaydı aç** ile geri yüklenir; yeniden analiz yeni revizyon oluşturur. CLI dosya dışa aktarma akışını korur.
