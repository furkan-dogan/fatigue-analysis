...
def edit_review(current):
    ...
    with st.container():
        ...
        st.caption('Bunlar kullanıcı gözlemidir; otomatik uygunluk veya doğruluk onayı değildir.')
        box = reference_fields('Sporcuyu referans karesinde kutuyla belirt', review['athlete_box'], metadata, key + '_box')
        st.caption('Sporcu kutusu bir referanstır; otomatik takip henüz yapılmaz.')
        ...
        table = st.data_editor(pd.DataFrame(rows, columns=columns), num_rows='dynamic', hide_index=True,
                               column_config={c: st.column_config.NumberColumn(labels[c], min_value=0, max_value=metadata['frame_count'] - 1, step=1, required=True) for c in columns},
    ...
    return dict(test=test, athlete=athlete, start_frame=int(start), end_frame=int(end),
                ...
                camera=camera, view=view, feet_visible=feet, physical_time_confirmed=physical, notes=notes)
